...
def get_loader(aug_type, args, two_crop=False, prefix='train', return_coord=False):
    ...
    if args.zip:
        ...
        #     train_prefix = prefix + ".zip@/"
        #     train_props_file = prefix + f"_{args.filter_strategy}.json"
        # NOTE(JiaKui Hu): for coco, we use "./coco/2017/val/" as image-root, 
        # NOTE(JiaKui Hu): "./coco/2017/annotations/*" as ann-root
        # TODO(JiaKui Hu): rpn_props?
        elif args.dataset == 'COCO':
            # prefix = 'train'
            prefix = 'train' # use val data
            train_ann_file = ''.join(['instances_', prefix, '2017.json'])
            train_prefix = prefix + ".zip@/"
            if args.ss_props:
                # train_props_file = prefix + f"_{args.filter_strategy}.json"
                train_props_file = "train.json"
            # elif args.rpn_props:
                # train_props_file = f"rpn_props_nms_{args.nms_threshold}.json"
